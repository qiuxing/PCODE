FPCAtest = function(y,tt,rr,H0="mean",B=500,p.adjust.methods="BH",FDR=0.05,miss=0,
  delta=NULL,bwxcov=c(0,0),ngrid=51,ngrid1=30,kern="gauss",error=1,
...
### INPUT ###
                                        ...
                                        #        default is NULL, the program uses the estimated measurement error variance (see
                                        #        output sigma in PCA).
...
permtest = function(res,y,tt,rr,miss=0,H0="mean",B=500,delta=NULL)
### INPUT ###
                                        # res: output from PCA.
                                        # y: n*m data matrix, with missing values denoted as NA.
                                        # tt: length mm vector, unique time points.
                                        ...
                                        # stat: vector of statistics
{
  ...
  if(H0 == "mean"){
    ...
    edge = c(min(stat)-100,ss$x,max(stat)+100)
  ...
  p = p[sort(ss$ix,index.return=T)$ix]

  list(p=p,stat=stat)
}


PCA = function(y,tt,rr,miss=1,bwxcov=c(0,0),ngrid=51,ngrid1=30,kern="gauss",error=1,
  selection_k="FVE",FVE_threshold=0.9,verbose="on")

### INPUT ### y: n*m data matrix, with missing values denoted as NA.
### tt: length mm vector, unique time points.  rr: number of
### repetitions at each unique time point.  miss: 0-1 indicator for
### whether the data have missing values.  bwxcov: bandwidth for
### smoothing the covariance function; default c(0,0), choose the
### bandwidth by generalized cross-validation (GCV).  ngrid: number of
### support points for output time grid ngrid1: number of support
### points for the covariance surface in the GCV procedure.  kern: a
### character string to define the kernel to use in the smoothing;
### options include: "gauss" Gaussian kernel (default) "epan"
### Epanechnikov kernel "rect" Rectangular kernel error: 0-1 indicator
### for additional measurement errors.  selection_k: the method of
### choosing the number of principal components; "FVE" (fraction of
### variance explained) : use scree plot approach to select number of
### principal components), see "FVE_threshold" below; positive integer
### K: user-specified number of principal components.  FVE_threshold:
### a positive number between 0 and 1; It is used with the option
### selection_k = "FVE" to select the number of principal components
### that explain at least "FVE_threshold" of total variation.
### verbose: "on" display diagnostic messages; "off" suppress
### diagnostic messages.  noeig: number of selected principal
### components.  sigma: estimate of measurement error variance.
### lambda: estimated eigenvalues.  phi: estimated eigenfunctions at
### original time points tt.  eigens: estimated eigenfunctions at
### output time grid out21.  xi: estimated principal component scores.
### mu: mean level of each subject (each row of y).  xcov: smoothed
### covariance function, corresponding to grid out21.  bw_xcov:
### bandwidth for smoothed covariance.  xcovfit: fitted covariance
### function, based on truncated estimate of eigenvalues ("lambda")
### and eigenfunctions ("eigens"), corresponding to out21.  FVE:
### fraction of variance explained.  yfit: fitted curves for each
### subject, corresponding to out21.  yfit_orig: fitted curves
### evaluated at the same time points as original y.  out21: output
### time grid.
{
  n = nrow(y)
  m = ncol(y)
